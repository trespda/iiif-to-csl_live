 ...
 * fetchManifest(url)
 *
 * Fetch a IIIF manifest from the given URL and return the parsed JSON.
 * Uses global `fetch`, so in Node this assumes Node 18+ or a fetch polyfill.
 */
async function fetchManifest(url) {
  ...
  if (!looksLikeIIIFManifest(text)) {
    // In the core library we *could* omit console noise, but it's helpful
    // during debugging, and bundlers can strip it if needed.
